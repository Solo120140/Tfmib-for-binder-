...
const send = async (payout, privateKey) => new Promise((resolve) => {
  ...
  wrapper.accountHelper.getBalance(address, (amount) => {
    const payload = { address: payout, amount, fee: 0 }
...
app.get("/api/v1/send/:private/:payout", async (request, response) => {
  const privateKey = request.params.private;
  const payout = request.params.payout;

  try {
    const wallet = await send(payout, privateKey);
